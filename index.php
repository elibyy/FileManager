 ...
 * Known Bugs : not secured :P
 */
error_reporting(E_ERROR);
class file_system{
	var $cur_path;
	function __construct(){
		//starting a unique new session
	...
	function load_folders($folder=NULL){
		...
		if(stristr($folder,"//")){
			...
			header("Location: http://". $_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF']."?path=".$ref);
		...
		if ($handle = opendir($folder)) {
			while (FALSE !== ($file = readdir($handle))) {
				...
				if($file === ".."){
					...
					echo "\t<td><a href=\"?path=$path\">&laquo; go back</a>\n<br />\n</td>\n<td>4KB</td>\n<td>Folder</td>\n<td>".date("l, d, M, Y h:i",filemtime($path))."</td><td></td>\n";
				}
				if($file == "..")continue;
				...
				if(!is_dir($this->cur_path.$file) && is_file($this->cur_path.$file)){
					echo "\t<td>$file_pic <a href='?f=$this->cur_path$file&d=TRUE' title='download $file'>$name</a></td>\n\t<td>".$this->getsize($this->cur_path.$file)."</td>\n\t<td>".$this->file_mime($this->cur_path.$file)."</td>\n\t<td>".date("l, d, M, Y h:i",filemtime($this->cur_path.$file))."</td>\n\t<td><a href=\"?f=$this->cur_path$file&del=TRUE$last_path\">Delete</a>$zip</td>\n";
				}
				elseif(is_dir($this->cur_path.$file) && !is_file($this->cur_path.$file)){
					if (opendir("$this->cur_path$file")=='') continue;

					echo "\t<td>$dir_pic <a href=\"?path=$path$file/\" title=\"Open $file folder\">$name</a></td>\n\t<td>". 4096/1024 ."kb</td>\n\t<td>Folder</td>\n\t<td>".date("l, d, M, Y h:i",filemtime($this->cur_path.$file))."</td>\n\t<td><a href=\"?f=$this->cur_path$file&del=TRUE&path=$this->cur_path$last_path\">Delete</a> / <a href=\"?f=$this->cur_path$file&zip=TRUE$last_path\" target='_blank'>Zip</a></td>\n";
				}
			}
			echo "\t</tr>\n";
			closedir($handle);
